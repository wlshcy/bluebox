...
def index(request):
    ...
    return response('items.html', {'items': Item.objects.order_by('-created')}, context_instance=RequestContext(request))
...
def show(request):
    ...
    return response('item.html',
                    {'id': item.id,'name': item.name, 'desc': item.desc,
                     ...
                     'mprice': item.mprice, 'origin': item.origin, 'photo': item.photo},
